...
$.when(getcolors, getcontacts, getregions, getcountries)
    .done(function() {
        ...
        function addLayer(collection) {
            $.each(collection.features,
                function(i, v) {
                    var name = v.properties.name;
                    v.properties.style = {};
                    v.properties.style.fillColor = colors[name];
        ...
        function onEachFeature(feature, layer) {
                ...
                if (c) {
                    ...
                    for (name in people) {
                        if (people.hasOwnProperty(name)) {
                            var contact = c[name];
                            list += '<h4>' + name + '</h4>' + '<p>' + contact.comment + '</p>';
            ...
            properties.popup = '<b class=popupheader>' + name + '</b>';

            if (c) {
                properties.popup = '<p><b class=popupheader>' + c.orgname + '</b>('+c.website+')</p>';
                $.each(c.people,
                    function(i, v) {
        ...
        var regionlayer = addLayer(regions);
